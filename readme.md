...
AOSI (AI Open Systems Interconnection) is inspired by the same principles of the OSI (Open Systems Interconnection) model. AI, LLMs, and autonomous agents are growing in complexity, with multiple layers of computation, data processing, orchestration, and interaction. Right now, the industry lacks a shared technical framework to describe these systems from the ground up. That’s where AOSI comes in.

AOSI provides a 7-layer technical reference model for AI systems, starting from the infrastructure that powers AI all the way to the applications that deliver intelligence to humans or other systems. Each layer has a single-word, descriptive name and a clear technical responsibility, making it easy to analyze, design, and discuss AI systems.

The goal of AOSI is simple: create a common vocabulary and functional roadmap for AI systems, so that developers, researchers, and organizations can communicate more clearly, design more reliably, and innovate faster—just like OSI did for networking.
...
AOSI is a **proposed draft**; we believe the best standards are built **collaboratively**. This reference model is intended as a **shared framework for the entire AI industry**, where researchers, developers, and vendors can **work together to define, refine, and evolve the model**.

We invite **all AI vendors, tool developers, and community members** to participate in shaping AOSI. The idea is to create a **working group-style collaboration** directly on GitHub, where:

* Each organization or individual can become a **contributor**.
